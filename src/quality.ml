...
let compute_quality_cost graph preprocess quality cost =
  ...
  let rec compute v =
    ...
    *)
    (quality v qualities, List.reduce (+.) costs +. (cost v))
  in
  ...
  FlowgraphDfs.prefix (fun vertex ->
      ...
      if not (List.is_empty preds) then
        begin
          ...
          let new_mark =  if fst_node.className = "resampler" then
          ...
          in
          G.Mark.set (List.hd preds) new_mark
        end
    )
...
let cost node =
  if G.Mark.get node = 1 then 0.5 else 1.0
